...
$("#btnCalculate").click(function() { // when this element is clicked call this function, JavaScript callback, one function calling another function.
	...
	// Put variables as whoel numbers because the decimals get confused as dots in the program.
	...
	// STEP #1: Convert variables into consistent units measurement.
	...
	var salesAmount = parseFloat($("#saleInput").val()); // takes the salesInput by user, converts from a string to a floating point number then multiplies it by 100 then stores it in salesAmount variable.

	// Total sales amount entered by user
	//Took the value from the customer amount input and converted from a string (number) to a integer floating point number then multiplied that value by 100 (making into cents units) then subtracted that amount by the salesAmount.
	...
	//check user input validation to make a number is chosen.
	if (isNaN(salesAmount) || isNaN(total_change) || (salesAmount < 0) || (customerAmount < 0)) {
		alert("Please enter a valid non-negative number to calculate change.");
		return;		
	}
	...
	//Taking sales amount, cutting off 2 decimals and seeing if it is equal to the original sales amount, which should only be different if there is more than 2 decimal places.
	//Therefore if it is not equal then we know there is more than 2 decimal places.
	//Any time there is more than 2 decimal places the toFixed amount is not equal to the original amount and we aler the user.
